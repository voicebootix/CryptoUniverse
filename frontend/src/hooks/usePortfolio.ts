...
export const usePortfolioStore = create<PortfolioState>((set) => ({
  ...
  fetchPortfolio: async () => {
    ...
    try {
      ...
      set(
        produce((draft: Draft<PortfolioState>) => {
  ...
  fetchStatus: async () => {
    ...
    try {
      ...
      set(
        produce((draft: Draft<PortfolioState>) => {
          if (
            data.performance_today &&
            Array.isArray(data.performance_today.history)
          ) {
            draft.performanceHistory = data.performance_today.history.map(
              (h: any) => ({
  ...
  connectWebSocket: () => {
    ...
    const wsUrl = import.meta.env.VITE_API_URL
      ? `wss://${import.meta.env.VITE_API_URL.replace("https://", "").replace(
          "/api/v1",
          ""
        )}/api/v1/trading/ws`
        ...
        set(
          produce((draft: Draft<PortfolioState>) => {
            if (data.type === "market_update") {
              ...
              const existingIndex = draft.marketData.findIndex(
              ...
              if (existingIndex >= 0) {
                draft.marketData[existingIndex] = {
                  ...
                  volume: data.data.volume_24h
                    ? `${(data.data.volume_24h / 1e6).toFixed(0)}M`
                    : "N/A",
                };
              } else {
              ...
              // Update portfolio value and performance history
              if (data.data.total_value_usd !== undefined) {
                draft.totalValue = data.data.total_value_usd;
              }
              if (data.data.daily_pnl !== undefined) {
                draft.dailyPnL = data.data.daily_pnl;
                draft.dailyPnLPercent = data.data.daily_pnl_percent || 0;
              }
              if (
                data.data.performance_history &&
                Array.isArray(data.data.performance_history)
              ) {
                draft.performanceHistory = data.data.performance_history.map(
                  (point: any) => ({
                    time: new Date(point.timestamp).toLocaleTimeString(),
                    value: point.portfolio_value_usd,
                  })
                );
              }
              ...
              const newPoint = {
                ...
                value: data.data.total_value_usd,
              };

              // Keep last 24 hours of data points (288 points at 5-minute intervals)
              const updatedHistory = [...draft.performanceHistory, newPoint]
                .slice(-288)
                .sort(
                  (a, b) =>
              ...
              draft.performanceHistory = updatedHistory;
            }
          })
        );
      } catch (error) {
