...
Ready to manage your crypto wealth! 💎
    ...
    COMMAND_PATTERNS = {
        ...
        'signal_config': r'^/signal_config(?:\s+([\w-]+))?',
        'signal_symbols': r'^/signal_symbols\s+([\w-]+)\s+(.+)',
        'signal_timeframe': r'^/signal_timeframe\s+([\w-]+)\s+(\w+)',
        'signal_autopilot': r'^/signal_autopilot\s+(on|off)(?:\s+([\w-]+))?',
        'signal_history': r'^/signal_history(?:\s+(\d+))?',
        'signal_performance': r'^/signal_performance(?:\s+([\w-]+))?',
        'ai': r'^/ai\s+(.+)',
        'help': r'^/help(?:\s+(\w+))?',
        'start': r'^/start',
