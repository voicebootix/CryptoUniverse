...
Enterprise-grade data persistence for real market data with enhanced models
and proper precision for financial calculations.
"""
...
from sqlalchemy import (
    ...
    Boolean, Text, Index, UniqueConstraint, ForeignKey, Numeric,
...
class OrderBookSnapshot(Base):
    ...
    # Order book data (stored as JSONB for performance)
    bids = Column(JSONB, nullable=False)  # [[price, amount], ...]
    asks = Column(JSONB, nullable=False)  # [[price, amount], ...]
...
class StrategyPerformanceHistory(Base):
    ...
    # Additional data
    traded_symbols = Column(JSONB)  # List of symbols traded
    trade_distribution = Column(JSONB)  # Distribution by hour/day
...
class BacktestResult(Base):
    ...
    symbols = Column(JSONB, nullable=False)  # List of symbols tested

    # Configuration
    strategy_params = Column(JSONB)  # Strategy-specific parameters
    risk_params = Column(JSONB)  # Risk management settings
    execution_params = Column(JSONB)  # Execution settings
    ...
    # Additional legacy metrics from HEAD
    annual_return = Column(Numeric(8, 4), nullable=True)
    ...
    trade_log = Column(JSONB)  # Detailed trade history
    monthly_returns = Column(JSONB)  # Monthly return breakdown
